...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const processRes = await axios.post(`${API_BASE}/process`, { file_paths: filePaths });

      setClusters(processRes.data.clusters);
      setElbowGraph(`${API_BASE}${processRes.data.elbow_graph}`);
      setSilhouetteGraph(`${API_BASE}${processRes.data.silhouette_graph}`);
    } catch (err) {
  ...
  const openFile = (fileName) => {
    const fileUrl = `${API_BASE}/files/${fileName}`;
  ...
  return (
    <div className="flex p-6">
      <div className="flex-1 bg-gray-100 p-6 rounded-lg shadow-md">
            ...
            <div className="flex flex-wrap gap-2">
              {fileThumbnails.map((thumb, index) =>
                thumb ? (
                  ...
                  <div
                    key={index}
                    className="w-16 h-16 bg-gray-300 text-center flex items-center justify-center rounded-md text-xs"
                  >
                    {files[index].name}
                  </div>
                )
        ...
        <button onClick={handleRefresh} className="bg-gray-600 text-white px-6 py-2 rounded-md ml-4">
          Refresh
        </button>
